...
const receipt = await client.getTransaction({ hash: transactionHash as never });
...
const execution = receipt.txExecutionResultName ??
  raw.consensus_data?.leader_receipt?.[0]?.execution_result;
...
if (!deployedCode || Object.keys(schema).length === 0) {
  throw new Error("Deployment has no readable code or schema.");
}
if (sha256(deployedCode) !== sha256(localCode)) {
...
console.log(JSON.stringify({
  ...
  schemaMethods: Object.keys(
    (schema as unknown as { methods?: Record<string, unknown> }).methods ?? {}
  )
}, null, 2));
